...
/**
 * @title CCTPAuctionHookData Library
 * @notice Library for parsing CCTPAuction hook data from CCTP burn messages
 *
...
library CCTPAuctionHookData {
    ...
    function _parseBidParameters(bytes29 _hookData) internal pure returns (AuctionParams memory _params) {
        ...
        _params = AuctionParams({
            _auction: _hookData.indexAddress(AUCTION_INDEX),
            _maxPrice: _hookData.indexUint(MAX_PRICE_INDEX, MAX_PRICE_LENGTH),
            _bidder: _hookData.indexAddress(BIDDER_INDEX),
            _prevTickPrice: _hookData.indexUint(PREV_TICK_PRICE_INDEX, PREV_TICK_PRICE_LENGTH),
            _innerHookData: bytes29(0)
        });

        uint256 _innerLength = hookLength - INNER_HOOK_DATA_INDEX;
        if (_innerLength > 0) {
            _params._innerHookData = _hookData.slice(INNER_HOOK_DATA_INDEX, _innerLength, 0);
        }
    }
}
